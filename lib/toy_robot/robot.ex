defmodule ToyRobot.Robot do
  ...
  @doc """
    Moves the robot back one space in the opposite direction it is facing.

    ## Examples

    iex> alias ToyRobot.Robot
    ToyRobot.Robot
    iex> robot = %Robot{north: 1, facing: :north}
    %Robot{north: 1, facing: :north}
    iex> robot |> Robot.back
    %Robot{north: 0}
  """
  def back(%Robot{facing: facing} = robot) do
    case facing do
      :north -> robot |> move_south
      :east -> robot |> move_west
      :south -> robot |> move_north
      :west -> robot |> move_east
    end
  end
